...
class trainer():
    def __init__(self, coordinate_Path):
        self.device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
        ...
        Path_train_player = Path(f"{save_Path}train_data.pt")
        Path_train_label = Path(f"{save_Path}train_label.pt")

        self.data = reader(coordinate_Path,save_Path)
        ...
        self.train_dataloader = DataLoader(training_data, batch_size=self.batchsize)
        self.test_dataloader = DataLoader(test_data, batch_size=self.batchsize)
    ...
    def eval_helper(self):
        ...
        return np.mean(losses), acc, pred_arr, label_arr
