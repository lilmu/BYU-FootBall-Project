...
def add_noise(array):
    ...
    noise_sample_X = np.random.default_rng().uniform(0.04,0.07, int(player_num))
    noise_sample_Y = np.random.default_rng().uniform(0.04,0.07, int(player_num))
...
class reader():
    def __init__(self,dic_path,save_path):
        #get file names
        file_list = []
        ...
        file_list.sort()
        train_dataSet = np.zeros((1,31))
        test_dataSet = np.zeros((1,31))
        train_test = 0

        for file in file_list:
            #read file
            file_array = np.load((dic_path + "/" + file))
            # the file that split test and train ---------- need to be changed -----------
            ...
            # find the player that split two teams
            index = split(file_array)

            #split X,Y,abel (team left)
            ...
            if train_test == 0:
                train_dataSet = np.append(train_dataSet,[data_1],0)
                train_dataSet = np.append(train_dataSet,[data_2],0)
            else:
                test_dataSet = np.append(test_dataSet,[data_1],0)
                test_dataSet = np.append(test_dataSet,[data_2],0)
        train_dataSet = np.delete(train_dataSet,0,0)
        test_dataSet = np.delete(test_dataSet,0,0)

        #augument
        for i in range(len(train_dataSet[:,0])):
            aug = add_noise(train_dataSet[i,:])
